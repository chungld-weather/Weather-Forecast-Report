...
if datetime.now() > VALIDATION_DEADLINE:
    # Ensure QApplication exists before showing a message box
...
# ... (Translation dictionaries remain the same) ...
...
class WeatherCrawlerApp(QWidget):
    ...
    def initUI(self):
        ...
        # --- LEFT PANEL ---
        left_vbox.addWidget(self._create_locations_group())
        left_vbox.addWidget(self._create_actions_group())
    ...
    # --- UI Creation Methods (No more API group) ---
    def _create_locations_group(self):
        ...
        self.lat_label, self.lon_label = QLabel("Lat:"), QLabel("Lon:")
        ...
        if "An Phu Office" in self.location_radios:
            self.location_radios["An Phu Office"].setChecked(True)

        self.toggle_gps_input()
        group.setLayout(layout)
        return group

    # ... Other UI creation methods remain largely the same ...
    ...
    def _generate_report(self, lang):
        ...
        if lat is None:
            return
        api_choice = "OpenMeteo"  # Default to OpenMeteo, or set as needed
        ...
        self._start_progress(f"{lang_map[lang][0]} data...")
        QApplication.setOverrideCursor(Qt.WaitCursor)

        try:
            # Step 1: Fetch Open-Meteo Forecast Data
            self._update_progress_status(
                f"{lang_map[lang][1]} forecast data...")
            weather_data, location_info = self._fetch_weather_data_openmeteo(
                lat, lon)
            if not weather_data:
                raise Exception("Weather forecast data fetch failed")

            # Step 2: Fetch Marine Data if applicable
            marine_data = None
            include_marine = (any(p in location_name_selected for p in ["Lan Tay", "Rong Doi"]) or
                              (self.gps_radio.isChecked() and self.gps_marine_checkbox.isChecked()))
            if include_marine:
                self._update_progress_status(
            ...
            # Step 3: Fetch Precipitation Map from OpenWeatherMap
            self._update_progress_status(
                f"{lang_map[lang][1]} precipitation map...")
            precipitation_map_image = self._create_precipitation_map_image(
                lat, lon)
            if not precipitation_map_image:
                print("Warning: Could not generate precipitation map.")

            # Step 4: Generate the appropriate PDF
            self._update_progress_status(
                f"{lang_map[lang][2]} PDF {lang_map[lang][3]}...")
            if lang == 'en':
                self.generate_pdf_report(
                    lat, lon, weather_data, location_info, pdf_name, marine_data, precipitation_map_image)
            else:
    ...
    # --- Integrated Map Generation Functions ---
    def _create_precipitation_map_image(self, lat, lon):
        """Orchestrates the creation of a complete precipitation map image with legend."""
        try:
            x_center, y_center = self._deg2num(lat, lon, zoom=6)
            timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")

            # Fetch current precipitation value for annotation
            weather_data = self._fetch_current_owm_weather(lat, lon)
            precip_value = weather_data.get('rain', {}).get('1h', 0)

            # Generate the map components
            stitched_map = self._fetch_tile_grid(
                "precipitation", x_center, y_center, zoom=6, radius=1)
            marked_map = self._mark_location(
                stitched_map, lat, lon, 6, x_center, y_center, 1)
            annotated_map = self._annotate_image(
                marked_map, "precipitation", precip_value, CONFIG['UNITS']['precipitation'])
            legend = self._draw_color_legend(
                'precipitation', width=annotated_map.width)

            # Combine map and legend
            final_image_pil = Image.new(
                "RGB", (annotated_map.width, annotated_map.height + legend.height), "white")
            final_image_pil.paste(annotated_map, (0, 0))
            final_image_pil.paste(legend, (0, annotated_map.height), legend)

            # Convert to ReportLab Image object
            with BytesIO() as buffer:
                final_image_pil.save(buffer, "PNG")
                buffer.seek(0)
                return Image(buffer, width=7.5*inch, height=(7.5 * final_image_pil.height / final_image_pil.width)*inch)
        except Exception as e:
    ...
    def _get_owm_tile_url(self, layer, x, y, z):
        return f"https://tile.openweathermap.org/map/{layer}_new/{z}/{x}/{y}.png?appid={self.api_key}"
    ...
    def _fetch_tile_grid(self, layer, x_center, y_center, z, radius=1):
        ...
        for dx in range(-radius, radius + 1):
            for dy in range(-radius, radius + 1):
                ...
                try:
                    overlay_resp = requests.get(self._get_owm_tile_url(
                        layer, xtile, ytile, z), timeout=10)
                    if overlay_resp.ok:
                        base.paste(Image.open(BytesIO(overlay_resp.content)).convert(
                            "RGBA"), (0, 0), Image.open(BytesIO(overlay_resp.content)).convert("RGBA"))
    ...
    def _annotate_image(self, img, layer_name, value, unit):
        ...
        draw.rectangle(
            (5, 5, bbox[2] + 15, bbox[3] + 15), fill=(255, 255, 255, 200))
    ...
    def _draw_color_legend(self, layer, width, height=30):
        ...
        for x in range(width):
            ...
            index_floor = int(index_float)
            t_local = index_float - index_floor
        ...
        for i in range(7):
            val = min_val + i * (max_val - min_val) / 6
            label = f"{val:.0f}"
            x_pos = int(i * (width - 1) / 6)
            bbox = draw.textbbox((0, 0), label, font=font)
            draw.text((x_pos - (bbox[2] - bbox[0]) / 2,
                      height + 2), label, font=font, fill="black")
        return legend_img

    def _fetch_current_owm_weather(self, lat, lon):
        """Dedicated function to fetch current weather from OWM for map annotation."""
    ...
    def generate_pdf_report(self, lat, lon, weather_data, location_info, ui_location_name, api_source, marine_data=None, precipitation_map_image=None):
        ...
        if precipitation_map_image:
            elements.append(Spacer(1, 0.2*inch))
            elements.append(
                Paragraph("Precipitation Map (from OpenWeatherMap)", styles['h2']))
            elements.append(precipitation_map_image)
