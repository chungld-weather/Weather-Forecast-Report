...
from PyQt5.QtWidgets import (QApplication, QWidget, QLabel, QLineEdit,
                             ...
                             QMessageBox, QDateEdit, QProgressBar, QSizePolicy)  # Added QSizePolicy
...
# --- In generate_vietnamese_pdf_report ---
# # Modify style definitions to use the bold font where appropriate:
# title_style.fontName = VIETNAMESE_FONT_NAME_BOLD # Use bold for title
# h2_style.fontName = VIETNAMESE_FONT_NAME_BOLD    # Use bold for h2
...
# # In the table style commands, update the header font:
# # ('FONTNAME', (0, 0), (-1, 0), VIETNAMESE_FONT_NAME + '-Bold'), # Original attempt
# ('FONTNAME', (0, 0), (-1, 0), VIETNAMESE_FONT_NAME_BOLD), # Use registered bold name
# # Make sure the rest of the table uses the regular font:
...
# ... (scrape_and_download_tidal_pdf_for_date function remains unchanged) ...
...
class WeatherCrawlerApp(QWidget):
    ...
    def initUI(self):
        ...
        # --- Modified Loop for Location Radio Buttons ---
        for name, coords in self.locations.items():
            location_hbox = QHBoxLayout()  # Create a horizontal layout for each location

            radio = QRadioButton(name)  # Radio button with just the name
            radio.setFont(location_font)
            self.location_radios[name] = radio  # Store the radio button itself
            ...
            # Align coordinates to the left (center vertical)
            coord_label.setAlignment(Qt.AlignCenter | Qt.AlignVCenter)

            # Add radio button to the hbox (Left)
            location_hbox.addWidget(radio)
            # location_hbox.addStretch() # Add stretch between name and coords
            # Add coordinate label (Center)
            location_hbox.addWidget(coord_label)
            # location_hbox.addStretch(1) # *** ADDED: Add stretch after coords (Right) ***

            # Add the horizontal layout to the main input layout
        ...
        gps_input_layout.addStretch()  # Add stretch here if you want inputs to stay left
        input_layout.addLayout(gps_input_layout)

        # --- Default Location Selection ---
        # Use a key known to exist, or the first key if unsure
        default_location_key = 'An Phu Office'
        if default_location_key not in self.location_radios:
            # Fallback to the first key in the dictionary
        ...
        # --- NEW: Generate Vietnamese Weather Report Button ---
        self.vietnamese_weather_button = QPushButton(
            "Tạo Báo Cáo Thời Tiết (Tiếng Việt)")  # Vietnamese Label
        self.vietnamese_weather_button.setFont(QFont("Arial", 10))
        self.vietnamese_weather_button.clicked.connect(
            self.generate_vietnamese_weather_report_action)  # Connect to new action
        ...
        # --- 7. Progress Bar ---
        # REMOVED: progress_layout = QHBoxLayout()
        ...
        # self.progress_bar.setMinimumWidth(self.weather_button.sizeHint().width()) # Match button width
        # Expanding width, fixed height
        self.progress_bar.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        # ADDED: Add progress bar directly to main layout
        main_layout.addWidget(self.progress_bar)
        # REMOVED: main_layout.addLayout(progress_layout)

        # --- 8. Status Label ---
        # Create status label instance variable
        self.status_label = QLabel("", self)
        self.status_label.setAlignment(Qt.AlignCenter)
        self.status_label.setFont(QFont("Arial", 9))
        self.status_label.setVisible(False)  # Initially hidden
        # ADDED: Add status label directly to main layout
    ...
    # ... (Keep generate_weather_report_action) ...
    def generate_weather_report_action(self):
        ...
        self.status_label.setText("Preparing to fetch weather data...")
        ...
        try:
            ...
            self.status_label.setText("Fetching weather data... Please wait")
            ...
            if not weather_data:
                ...
                # Use return here, no need to proceed
        ...
        # --- NEW: Action for Vietnamese Weather Report Button ---
    def generate_vietnamese_weather_report_action(self):
        ...
        self.status_label.setText(
            "Đang chuẩn bị lấy dữ liệu thời tiết...")  # Vietnamese
        self.status_label.setVisible(True)
        QApplication.processEvents()
        ...
        try:
            print("--- Starting Weather Data Fetch for Vietnamese Report ---")

            # Update status message
            self.status_label.setText(
                "Đang lấy dữ liệu thời tiết... Vui lòng chờ")  # Vietnamese
            QApplication.processEvents()
    ...
    def download_tidal_report_action(self):
        ...
        self.progress_bar.setVisible(True)
        self.status_label.setVisible(True)
        self.status_label.setText("Downloading tide report...")

        QApplication.processEvents()
        try:
            pdf_path, pdf_url = scrape_and_download_tidal_pdf_for_date(
                self.session, selected_date)
            if not pdf_path:
                QMessageBox.warning(
                    self,
                    "Tide Report Not Found",
                    f"Could not find the URL for the tidal PDF report for {selected_date.strftime('%d/%m/%Y')}.\n\n"
                    "You can also check manually at:\nhttp://www.kttv-nb.org.vn/index.php/thong-tin-kttv/thuy-van"
                )
                self.progress_bar.setVisible(False)
                self.status_label.setVisible(False)
                return
    ...
    def get_coordinates(self):
        if self.gps_radio.isChecked():
            try:
                ...
                # Keep coords here for internal use
    ...
    def fetch_weather_data(self, lat, lon):
        ...
        try:
            ...
            location_info['name'] = current_data.get(
                'name', f"Coords ({lat:.4f}, {lon:.4f})")  # Store API name
        ...
        for item in data.get('list', []):
            try:
                ...
                if local_time >= now_local - timedelta(hours=3) and local_time <= now_local + timedelta(days=5, hours=3):
                    ...
                    # Probability of Precipitation (%) # Default to 0.0, convert to %, round
    ...
    # --- MODIFIED create_temp_humidity_chart ---
    ...
    # --- MODIFIED create_rain_pop_chart ---
        # --- MODIFIED create_rain_pop_chart ---

    def create_rain_pop_chart(self, data, date_key='datetime_obj',
                              xlabel_text='Date/Time', rain_ylabel='Rainfall (mm/3h)',
    ...
    # ... (Keep generate_pdf_report - it already calls the chart functions) ...
    ...
    def generate_pdf_report(self, lat, lon, weather_data, location_info, ui_location_name):
        ...
        if ui_location_name.startswith("Custom GPS"):
            file_name_location = "Custom_GPS"
        else:
        ...
        is_api_name_useful = api_name and api_name != ui_location_name and not api_name.startswith(
            "Coords (")
        if ui_location_name.startswith("Custom GPS"):
            location_string = "<b>Location:</b> Custom GPS"
            if is_api_name_useful:
                location_string += f" ({api_name}"
                if country_code and country_code != 'N/A':
                    location_string += f", {country_code}"
                location_string += ")"
            elif country_code and country_code != 'N/A':
                location_string += f" ({country_code})"
        else:
        ...
        # --- Weather Forecast Table (Keep as is) ---
        if weather_data:
            elements.append(
            ...
            ], 'wind_gust': [], 'rain': [], 'visibility': [], 'humidity': [], 'pop': []}  # Added humidity, pop
            param_to_col_index = {
                'temperature': 2, 'humidity': 3, 'wind_speed': 4, 'wind_gust': 5, 'rain': 7, 'pop': 8, 'visibility': 9
            }
            ...
            weather_col_widths = [1.5*inch, 1.68*inch, 0.75*inch, 0.8*inch,
            ...
            weather_style_cmds = [('BACKGROUND', (0, 0), (-1, 0), colors.darkblue), ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke), ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'), ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'), ('FONTSIZE', (0, 0), (-1, 0), 10.5), ('BOTTOMPADDING', (0, 0), (-1, 0), 8), ('TOPPADDING', (0, 0), (-1, 0), 6), ('FONTNAME',
                                                                                                                                                                                                                                                                                                                                                                                        (0, 1), (-1, -1), 'Helvetica'), ('FONTSIZE', (0, 1), (-1, -1), 10), ('TOPPADDING', (0, 1), (-1, -1), 5), ('BOTTOMPADDING', (0, 1), (-1, -1), 5), ('GRID', (0, 0), (-1, -1), 0.5, colors.grey), ('ALIGN', (2, 1), (3, -1), 'RIGHT'), ('ALIGN', (4, 1), (5, -1), 'RIGHT'), ('ALIGN', (7, 1), (9, -1), 'RIGHT'), ('ROWBACKGROUNDS', (0, 1), (-1, -1), [colors.whitesmoke, colors.lightblue])]
            ...
            elements.append(Paragraph(
                "Note: 1 knot = 1.85 km/h; Wind Dir = Wind Direction; PoP = Probability of Precipitation, the likelihood of rain or snow.", small_note_style))
        ...
        if weather_data:
            ...
            # # --- ADDED: Generate and add combined Temp/Humidity chart ---
            # temp_hum_chart = self.create_temp_humidity_chart(weather_data)
            # if temp_hum_chart:
            #     elements.append(temp_hum_chart)
            #     elements.append(Spacer(1, 0.1*inch))
            #     charts_added += 1
            # --- END ADDED ---
            # --- Generate combined wind chart (Keep as is) ---
            ...
            # Check if any charts were added (optional, for 'no data' message)
            ...
            # # Generate remaining individual charts (only visibility now)
    ...
    # --- END MODIFIED generate_pdf_report ---
        ...
        # --- NEW: Generate Vietnamese PDF Report ---
    def generate_vietnamese_pdf_report(self, lat, lon, weather_data, location_info, ui_location_name):

        page_width, page_height = landscape(letter)

        # --- Filename Logic (Vietnamese) ---
        if ui_location_name.startswith("Custom GPS"):
            file_name_location = "GPS_TuyChinh"
        else:
            # Basic transliteration/removal for filename safety
            name_map = {'Đ': 'D', 'đ': 'd'}
        ...
        # Vietnamese filename
        file_name = f"BaoCao_ThoiTiet_{file_name_location}_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
 ...
 # --- Define Styles (Get base styles first) ---
        ...
        # --- Report Header (Translated) ---
        elements.append(Paragraph("Báo Cáo Dự Báo Thời Tiết",
                        title_style))  # Vietnamese
        elements.append(Spacer(2, 0.1*inch))

        # Location string logic (same as English version)
        api_name = location_info.get('name', '')
        country_code = location_info.get('country', 'N/A')
        is_api_name_useful = api_name and api_name != ui_location_name and not api_name.startswith(
            "Coords (")
        if ui_location_name.startswith("Custom GPS"):
            location_string = "<b>Vị trí:</b> GPS Tùy Chỉnh"  # Vietnamese
            if is_api_name_useful:
                location_string += f" ({api_name}"
                if country_code and country_code != 'N/A':
                    location_string += f", {country_code}"
            ...
            if location_details_parts:
                location_details = ", ".join(location_details_parts)
                # Vietnamese
                location_string = f"<b>Vị trí:</b> {ui_location_name} ({location_details})"
            else:
                # Vietnamese
        ...
        if weather_data:
            elements.append(
                Paragraph("Dự Báo Thời Tiết (5 Ngày Tới)", h2_style))  # Vietnamese
            # Translated Headers
            ...
            for row_idx, item in enumerate(weather_data):
                ...
                viet_desc = WEATHER_DESC_VIET.get(eng_desc, eng_desc)
                # Fallback to English if not found
            ...
            weather_style_cmds = [('BACKGROUND', (0, 0), (-1, 0), colors.darkblue), ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke), ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
                                  ...
                                  # Bold header if font supports it
                                  ('FONTNAME', (0, 0), (-1, 0),
                                   VIETNAMESE_FONT_NAME + '-Bold'),
            ...
            # Translated Note
            elements.append(Paragraph(
                " Ghi chú: 1 knot = 1.85 km/h; Hướng gió = Hướng gió thổi tới; XS mưa = Xác suất có mưa hoặc tuyết.", small_note_style))  # Vietnamese
            elements.append(Spacer(1, 0.15*inch))
        else:
            elements.append(
