...
# Inside the WeatherCrawlerApp class
    ...
    def generate_weather_report_action(self):
        ...
        try:
            ...
            # No need to set value for indeterminate bar, but keep for text
    ...
    #         # Set major ticks to occur specifically at 01:00 and 13:00 each day
    #         # *** MODIFIED LINE ***
    #         plt.gca().xaxis.set_major_locator(
    #             mdates.HourLocator(byhour=[1, 13]))
    #         # Format major ticks to show Month-Day and the Hour:Minute
    #         plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%m-%d %H:%M'))

    #         # Keep minor ticks every 6 hours for context (optional)
    ...
    # In Weather & Tide Reporter 12 Apr.py
# Inside the WeatherCrawlerApp class
...
# Inside the WeatherCrawlerApp class
    ...
    def create_chart(self, data, param_name, ylabel, title, date_key='datetime_obj'):
        # ... (Keep the start of the function: figure creation, data extraction) ...
        if not data:
            print(f"Warning: No data provided for chart: {title}")
            return None
