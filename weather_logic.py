...
class WeatherLogic:
    ...
    def generate_pdf_report(self, lat, lon, weather_data, location_info, ui_location_name, api_source, marine_data=None, precipitation_map=None):
            ...
            if weather_data:
                ...
                for param, data_points in numeric_data_for_highlight.items():
                    if data_points:
                        ...
                        # Add yellow highlights for extremes (overriding min/max if needed)
                        for val, r_idx in data_points:
                            if param == 'temperature' and float(val) > 37:
                                weather_style_cmds.append(('BACKGROUND', (col_idx, r_idx), (col_idx, r_idx), colors.yellow))
                            elif param == 'wind_speed' and float(val) > 25:
                                weather_style_cmds.append(('BACKGROUND', (col_idx, r_idx), (col_idx, r_idx), colors.yellow))
                            elif param == 'rain' and float(val) > 0.5:
                                weather_style_cmds.append(('BACKGROUND', (col_idx, r_idx), (col_idx, r_idx), colors.yellow))
                            elif param == 'uv_index' and float(val) > 10:
                                weather_style_cmds.append(('BACKGROUND', (col_idx, r_idx), (col_idx, r_idx), colors.yellow))
                ...
                if len(marine_table_data) > 1:
                    ...
                    for param, data_points in marine_numeric_data_for_highlight.items():
                        if data_points:
                            ...
                            for val, r_idx in data_points:
                                if param == 'wave_height' and float(val) > 2.5:
            ...
            if marine_data:
                elements.append(
                    Paragraph(f"Weather data © {api_source}, Marine data © Open-Meteo", footer_style))
            else:
                elements.append(
                    Paragraph(f"Weather data © {api_source}", footer_style))
            elements.append(
    ...
    def generate_vietnamese_pdf_report(self, lat, lon, weather_data, location_info, ui_location_name, api_source, marine_data=None, precipitation_map=None):
            ...
            if marine_data:
                elements.append(
                    Paragraph(f"Dữ liệu thời tiết © {api_source}, Dữ liệu biển © Open-Meteo", footer_style))
            else:
                elements.append(
                    Paragraph(f"Dữ liệu thời tiết © {api_source}", footer_style))
            elements.append(Paragraph(
